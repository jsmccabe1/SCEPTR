...
def main():
    ...
    if args.continuous and not args.no_continuous:
        logger.info("Computing continuous enrichment functions...")

        # Build membership matrix from already-categorised data
        ...
        for cat, s in shape_stats.items():
            if s['max_enrichment'] > 1.5:
                logger.info(f"  {cat}: {s['shape_class']} "
                            f"(slope={s['slope']:.3f}, max={s['max_enrichment']:.2f})")

        # Global profile test (permutation-based)
    ...
    try:
        interactive_report.save_report_data(
            ...
            f"{data_prefix}_BP_MF_report_data.json")
    except Exception as e:
